...
// 쿠키 이름은 여기 한 곳에서만 정의한다. 문자열을 각 서비스에 흩뿌리면 한 곳만 어긋나도
// 컴파일 에러 없이 로그인만 조용히 실패한다.
export const COOKIE_SESSION = "ev_session";  // HttpOnly JWT — 인증의 유일한 근거
export const COOKIE_USER = "ev_user";        // 프론트가 읽는 { name, role } (UI 표시 전용)
...
// 세션 쿠키 한 쌍(HttpOnly JWT + 프론트용 표시 정보)을 만든다. 로그인·슬라이딩 갱신·
// 역할 동기화가 모두 같은 형태를 써야 하므로 한 곳에서 생성한다. maxAge=0이면 삭제용.
export function sessionCookies({ jwt, name, role }, maxAge, isSecure) {
  const opts = formatCookieOpts(maxAge, isSecure);
  const userPayload = maxAge === 0 ? "" : encodeURIComponent(JSON.stringify({ name, role }));
  return [
    `${COOKIE_SESSION}=${maxAge === 0 ? "" : jwt}; HttpOnly; ${opts}`,
    `${COOKIE_USER}=${userPayload}; ${opts}`,
  ];
}
  ...
  app.use(async (req, res, next) => {
    ...
    if (token && process.env.JWT_SECRET) {
      try {
        ...
        if (remaining < threshold) {
          ...
          const jwt = createJWT({ email, name, role }, process.env.JWT_SECRET);
          res.setHeader("Set-Cookie", sessionCookies({ jwt, name, role }, 7 * 24 * 3600, isSecureConnection(req)));
        }
      } catch { /* invalid token */ }
    }
    ...
    if (req.user && validateUser) {
      ...
      if (!valid) {
        ...
        const jwt = createJWT({ email, name, role: freshRole }, process.env.JWT_SECRET);
        res.setHeader("Set-Cookie", sessionCookies({ jwt, name, role: freshRole }, 7 * 24 * 3600, isSecureConnection(req)));
      }
    }
